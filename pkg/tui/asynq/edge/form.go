...
import (
	...
	"github.com/Ahu-Tools/ahum/pkg/asynq/edge"
...
func NewForm(pj project.ProjectInfo) *Form {
	...
	form := huh.NewForm(
		huh.NewGroup(
			huh.NewInput().
				Title("How many concurrent worker do you want to setup?").
				Validate(numValid).
				Value(&concurrency).
				Key("concurrency"),
		),
...
func (f Form) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case huh.StateCompleted:
		...
		concurrency, _ := strconv.Atoi(f.form.GetString("concurrency"))
		cfg := edge.Config{
